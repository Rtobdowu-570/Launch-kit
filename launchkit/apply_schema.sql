-- Run this SQL in your Supabase SQL Editor to create the brands table
...
-- Create RLS policies for brands (allow all for now, refine later)
...
CREATE POLICY "Enable read access for all users" ON public.deployments FOR SELECT USING (true);

DROP POLICY IF EXISTS "Enable insert for all users" ON public.deployments;
CREATE POLICY "Enable insert for all users" ON public.deployments FOR INSERT WITH CHECK (true);
